app.get("/dashboard", async (req, res) => {
  ...
  if (ADMINS.includes(email)) {
    try {
          ...
          <div style="max-width: 1000px; margin: auto; padding: 40px;">
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-bottom: 40px;">
              <div style="background: #1e293b; padding: 30px; border-radius: 15px; border: 1px solid #334155;">
                <h3 style="margin:0; color: #94a3b8; font-size: 14px; text-transform: uppercase;">Total Repair Units</h3>
                <p style="font-size: 48px; margin: 10px 0 0 0; font-weight: bold;">${stats.rows[0].total}</p>
              </div>
              <div style="background: #1e293b; padding: 30px; border-radius: 15px; border: 1px solid #334155;">
                <h3 style="margin:0; color: #94a3b8; font-size: 14px; text-transform: uppercase;">Total Revenue</h3>
                <p style="font-size: 48px; margin: 10px 0 0 0; font-weight: bold; color: #22c55e;">$${stats.rows[0].revenue || 0}</p>
              </div>
            </div>
      ...
      res
        .status(500)
        .send("Database Error: Verify Environment Variables on Render.");
    }
  } else {
